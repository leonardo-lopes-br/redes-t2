...
class Servidor:
    ...
    def _rdt_rcv(self, src_addr, dst_addr, segment):
        ...
        if (flags & FLAGS_SYN) == FLAGS_SYN:
            ...
            header = make_header(dst_port, src_port, seq_number, seq_no + 1, FLAGS_SYN | FLAGS_ACK)
            packed_header = fix_checksum(header, dst_addr, src_addr)
            self.rede.enviar(packed_header, src_addr)
            if self.callback:
...
class Conexao:
    def __init__(self, servidor, id_conexao, seq_number_server, ack_number):
        self.servidor = servidor
        self.id_conexao = id_conexao
        self.ack_number = ack_number
        self.seq_number_server = seq_number_server
        self.callback = None
